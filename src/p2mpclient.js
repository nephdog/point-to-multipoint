...
const hostnames = process.argv.slice(2, process.argv.length - 4);
const port = Number(process.argv[process.argv.length - 4]);
const file = path.resolve(__dirname , '../' + process.argv[process.argv.length -3]);
const mss = Number(process.argv[process.argv.length -2]);
const clientHost = process.argv[process.argv.length - 1];
...
const sender = new Sender(hostnames, port, file, mss, clientHost);
sender.start()
